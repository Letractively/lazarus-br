...
uses {$IFDEF LINUX}Libc,{$ENDIF} dynlibs, ctypes, os_types;
...
initialization
  ...
  Libhandle := LoadLibrary(libao);
  if (Libhandle <> 0) then
  begin
    ao_initialize :=Tao_initialize(GetProcAddress(Libhandle,'ao_initialize'));
    ao_shutdown := Tao_shutdown(GetProcAddress(Libhandle,'ao_shutdown'));
    ao_append_option := Tao_append_option(GetProcAddress(Libhandle,'ao_append_option'));
    ao_free_options := Tao_free_options(GetProcAddress(Libhandle,'ao_free_options'));
    ao_open_live := Tao_open_live(GetProcAddress(Libhandle,'ao_open_live'));
    ao_open_file := Tao_open_file(GetProcAddress(Libhandle,'ao_open_file'));
    ao_play := Tao_play(GetProcAddress(Libhandle,'ao_play'));
    ao_close := Tao_close(GetProcAddress(Libhandle,'ao_close'));
    ao_driver_id := Tao_driver_id(GetProcAddress(Libhandle,'ao_driver_id'));
    ao_default_driver_id := Tao_default_driver_id(GetProcAddress(Libhandle,'ao_default_driver_id'));
    ao_driver_info :=Tao_driver_info(GetProcAddress(Libhandle,'ao_driver_info'));
    ao_driver_info_list := Tao_driver_info_list(GetProcAddress(Libhandle,'ao_driver_info_list'));
    ao_file_extension := Tao_file_extension(GetProcAddress(Libhandle,'ao_file_extension'));
    ao_is_big_endian := Tao_is_big_endian(GetProcAddress(Libhandle,'ao_is_big_endian'));
  end;
