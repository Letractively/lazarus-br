...
type
  ...
  TKeyFoundEvent = procedure(Sender: TObject; Key, Results, OriginalLine: string) of object;
  TKeyFoundExEvent = procedure(Sender: TObject; Key, Results, OriginalLine: string;
  ...
  private
    ...
    FContent: string;
    FOnKeyFound: TKeyFoundEvent;
    FOnKeyFoundEx: TKeyFoundExEvent;
  ...
  published
    ...
    property OnKeyFound: TKeyFoundEvent read FOnKeyFound write FOnKeyFound;
    property OnKeyFoundEx: TKeyFoundExEvent read FOnKeyFoundEx write FOnKeyFoundEx;
  end;

function ChaIsInSet(const Ch: AnsiChar; const SetOfChar: TSysCharSet): Boolean;  inline;
...
const
  AnsiCharCount = Ord(High(AnsiChar)) + 1;
  AnsiCaseMapSize = AnsiCharCount * 3;
  AnsiUpOffset = AnsiCharCount * 1;

var
  AnsiCaseMap: array [0..AnsiCaseMapSize - 1] of AnsiChar;
...
function ChaIsInSet(const Ch: AnsiChar; const SetOfChar: TSysCharSet): Boolean;
begin
...
procedure THTMLParser.Loaded;
begin
  inherited Loaded;
  SetParser(FParser);
end;

procedure THTMLParser.SetFileName(Value: TFileName);
begin
...
function THTMLParser.GetParser: TStrings;
begin
  Result := FParser;
end;

procedure THTMLParser.SetParser(Value: TStrings);
var
  I: Integer;
  Obj: TParserInfo;
  Cap: string;
begin
  if FParser <> Value then // make sure we don't assign to ourselves
...
procedure THTMLParser.AnalyseFile;
var
  List: TStringList;
begin
...
procedure THTMLParser.AnalyseString(const Str: string);
var
  Str2, Str3: string;
  ...
  begin
    ...
    QuoteChar := #0;
    repeat
      while ChaIsInSet(Tail^, [#13, #10, ' ']) do
        Inc(Tail);
      Head := Tail;
      InQuote := False;
      LeadQuote := False;
      while True do
      begin
        while (InQuote and not ChaIsInSet(Tail^, [#0, '"'])) or
              not ChaIsInSet(Tail^, [#0, #13, #10, ' ', '"']) do
          Inc(Tail);
        if Tail^ = '"' then
        begin
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        StartTag1 := TParserInfo(FKeys.Objects[I]).StartTag;
        Starttag2 := '';
        ...
        begin
          ...
          begin
            // changed from StrSearch(case sensitive) to StrFind (case insensitive)
            K := StrFind(TParserInfo(FKeys.Objects[I]).EndTag, Str, J);
            TagInfo.BeginPos := J;
            TagInfo.EndPos := K + Length(TParserInfo(FKeys.Objects[I]).EndTag);
            TagInfo.Key := I;
            case TParserInfo(FKeys.Objects[I]).TakeText of
              0: //Between limits
                begin
                  if StartTag2 = '' then
                    TagInfo.BeginContext := J + Length(TParserInfo(FKeys.Objects[I]).StartTag)
                  else
                    // changed from StrSearch(case sensitive) to StrFind (case insensitive)
              ...
              3: //The whole line if containing start tag
                begin
                  ...
                  // changed from StrSearch(case sensitive) to StrFind (case insensitive)
                  TagInfo.EndContext := StrFind(#10, Str, J);
                end;
            end;
            FTagList.AddValue(TagInfo);
            J := J + Length(TParserInfo(FKeys.Objects[I]).StartTag);
          end;
        end;
      end;
    end;
...
procedure THTMLParser.AddCondition(const Keyword: string;
...
procedure THTMLParser.RemoveCondition(Index: Integer);
begin
  FKeys.Objects[Index].Free;
  FKeys.Delete(Index);
end;

procedure THTMLParser.ClearConditions;
var
  Index: Integer;
begin
  FParser.Clear;
...
procedure THTMLParser.GetCondition(Index: Integer; var Keyword, StartTag, EndTag: string);
begin
  Keyword := FKeys[Index];
  StartTag := TParserInfo(FKeys.Objects[Index]).StartTag;
  EndTag := TParserInfo(FKeys.Objects[Index]).EndTag;
end;
...
begin
  ...
  TextSelection := TParserInfo(FKeys.Objects[Index]).TakeText;
end;

function THTMLParser.GetConditionsCount: Integer;
begin
  Result := FKeys.Count;
end;

procedure THTMLParser.SetTagList(const Value: TTagInfoList);
begin
  FTagList := Value;
end;
